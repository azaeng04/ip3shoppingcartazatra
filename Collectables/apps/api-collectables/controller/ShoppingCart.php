...
<link href="controller/tableStyling.css" rel="stylesheet" type="text/css" />

<?php
define('__ROOTShoppingCart__', dirname(__DIR__)); 
require_once(__ROOTShoppingCart__ . '/db-layer/DBHander.php'); 
...
{      
   ...
   private function slugifyStoreId($storeId)
   {
       $slug = strtolower(trim($storeId));
   ...
   {
       $basePath = "images/images/";
       $fileFormat  = ".jpg";
       $storePath = $this->slugifyStoreId($this->storeID);
       $path = $basePath . $storePath . "/";
       if ($value['imageURL'] != null)
       {
           return ($path . $value['imageURL'] . $fileFormat);
       }
       else
            return ($basePath . "image_not_available" . $fileFormat);
   }
    ...
    public  function addItem()
    {
        if(!$this->refreshed() && !isset($_GET['btnNo']) && isset($_GET['ItemToAdd']))
        {
            ...
            if (isset($_SESSION['Last_ID']) && ($_SESSION['Last_ID'] === $ID))
            {
                ...
                $promptURL = "/controller/AddItemPrompt.php?productName=" . $this->inventory[$ID]['prodName'] . "&ID=" . $ID;
